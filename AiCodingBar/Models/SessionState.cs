...
public class SessionState
{
    ...
    /// <summary>来源 agent 标识 (claude-code / opencode / ...)</summary>
    public string AgentId { get; set; } = "unknown";

    /// <summary>Agent 在任务栏的缩写显示名</summary>
    public string AgentAbbr => AgentDisplay.GetAbbr(AgentId);

    /// <summary>Agent 在任务栏的完整显示名</summary>
    public string AgentName => AgentDisplay.GetName(AgentId);
...
/// <summary>
/// Agent 显示名映射表（agent_id → 缩写 + 全名）
/// 后续新增 agent 只需在此表中追加条目
/// </summary>
public static class AgentDisplay
{
    private static readonly Dictionary<string, (string Abbr, string Name)> _map = new()
    {
        ["claude-code"] = ("C", "Claude"),
        ["opencode"]    = ("O", "Open"),
    };

    public static string GetAbbr(string agentId) =>
        _map.TryGetValue(agentId, out var v) ? v.Abbr : "?";

    public static string GetName(string agentId) =>
        _map.TryGetValue(agentId, out var v) ? v.Name : agentId;

    public static bool IsKnown(string agentId) => _map.ContainsKey(agentId);
}
