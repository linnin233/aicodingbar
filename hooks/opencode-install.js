#!/usr/bin/env node
// Register AiCodingBar's opencode plugin in the user's global opencode config.
//
...
function resolvePluginDir(baseDir) {
  return path.resolve(baseDir || __dirname, PLUGIN_DIR_NAME).replace(/\\/g, "/");
}
...
function registerOpencodePlugin(options = {}) {
  ...
  if (!options.configPath) {
    ...
    if (!exists) {
      if (!options.silent) {
        console.log("AiCodingBar: ~/.config/opencode/ not found — skipping opencode plugin");
      }
  ...
  for (let i = 0; i < settings.plugin.length; i++) {
    ...
    if (entry === pluginDir) { matchIndex = i; break; }
  ...
  if (!skipped) writeJsonAtomic(configPath, settings);

  if (!options.silent) {
    console.log(`AiCodingBar opencode plugin → ${configPath}`);
